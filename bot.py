...
@app.route('/')
def home():
    return "Bot XAU Final - Text Only - No Terminal - Ready - /check /getid /getupdates"

@app.route('/check')
def check():
    return {
        "bot_token_set": bool(os.getenv("BOT_TOKEN")),
        "bot_token_preview": (os.getenv("BOT_TOKEN")[:10] + "..." ) if os.getenv("BOT_TOKEN") else "NOT SET",
...
@app.route('/getid')
def getid():
    # Cek bot info via HTTP Telegram API
    token = os.getenv("BOT_TOKEN")
    if not token or token == "dummy":
        return {"error": "BOT_TOKEN not set"}, 500
    try:
...
def getupdates():
    ...
    try:
        ...
        # ambil chat id terakhir
        if r.get("ok") and r.get("result"):
            last = r["result"][-1] if r["result"] else None
            chat_id = None
            if last:
                chat_id = last.get("message", {}).get("chat", {}).get("id") or last.get("channel_post", {}).get("chat", {}).get("id")
            return {"raw": r, "last_chat_id": chat_id, "hint": f"Copy last_chat_id ini ke ENV TARGET_CHAT_ID di Render"}
        return r
    except Exception as e:
        return {"error": str(e)}, 500
...
if __name__ == "__main__":
    # Fix 409 Conflict: hapus webhook + stop polling lain
    try:
        bot.remove_webhook()
        time.sleep(1)
        print("Webhook removed to fix 409")
    except:
        pass
    threading.Thread(target=run_flask, daemon=True).start()
    threading.Thread(target=auto_push_loop, daemon=True).start()
    print("Bot final jalan...")
    # skip_pending True biar gak bentrok
    while True:
        try:
            bot.infinity_polling(skip_pending=True, timeout=20, long_polling_timeout=20)
        except Exception as e:
            print(f"Polling error (biasa 409): {e} - retry 5 detik")
            time.sleep(5)
