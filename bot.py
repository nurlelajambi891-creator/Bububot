"""
BOT FINAL TERLENGKAP KSA - FIX BACKTEST KELUAR DI TELEGRAM
Fix: Backtest REAL sekarang keluar hasil di Telegram, bukan cuma suruh buka browser
- /backtest 150 candle 1day = 1 credit
- /backtest_scalping 200 candle 5m = 2 credit (sebelumnya 1000 = 10 credit boros)
- Tombol Backtest REAL di Telegram langsung jalan dan ngasih hasil text
...
CACHE = {"data": None, "time": 0, "analisa": None, "scalp": None, "backtest": None, "backtest_time": 0}
CACHE_TTL = 60
...
def td_get(symbol, endpoint="price", interval="1day", outputsize=30):
    try:
        ...
        if "code" in r and r.get("code") != 200:
            # error dari TwelveData
            print(f"TD ERROR {symbol} {interval}: {r}")
            if "values" not in r:
                return {"error": r}
        return r
...
def get_all_data():
    ...
    fvg_bull_15 = []; fvg_bear_15 = []; fvg_bull_5 = []; fvg_bear_5 = []
    try:
        if "values" in xau_15m:
            vals = xau_15m["values"][:15]
            for i in range(1, len(vals)-1):
                if float(vals[i-1]["low"]) > float(vals[i+1]["high"]): fvg_bull_15.append(float(vals[i-1]["low"]))
                if float(vals[i-1]["high"]) < float(vals[i+1]["low"]): fvg_bear_15.append(float(vals[i-1]["high"]))
        if "values" in xau_5m:
            vals = xau_5m["values"][:15]
            for i in range(1, len(vals)-1):
                if float(vals[i-1]["low"]) > float(vals[i+1]["high"]): fvg_bull_5.append(float(vals[i-1]["low"]))
                if float(vals[i-1]["high"]) < float(vals[i+1]["low"]): fvg_bear_5.append(float(vals[i-1]["high"]))
    except:
        pass
    daily_range = today_high - today_low if today_high > today_low else 20
    q1 = today_low + daily_range * 0.25; q2 = today_low + daily_range * 0.5; q3 = today_low + daily_range * 0.75
    now_ksa = datetime.now(KSA_TZ); hour = now_ksa.hour
    ...
    swept_5m_bull = False; swept_5m_bear = False
    try:
        if "values" in xau_5m and len(xau_5m["values"]) > 5:
            ...
            swept_5m_bull = live > last5_high; swept_5m_bear = live < last5_low
    except:
        pass
    score_buy = 0; score_sell = 0; notes = []
    if dxy < 103.5: score_buy += 1; notes.append(f"[BUY] DXY Lemah {dxy:.2f} <103.5")
    else: score_sell += 1; notes.append(f"[SELL] DXY Kuat {dxy:.2f} >103.5")
    ...
    if real_yield < 1.5: score_buy += 1; notes.append(f"[BUY] Real Yield Rendah {real_yield:.2f}%")
    else: score_sell += 1; notes.append(f"[SELL] Real Yield Tinggi {real_yield:.2f}%")
    ...
    if not swept_bsl and not swept_ssl: notes.append("[WAIT] Belum Sweep BSL/SSL")
    ...
    if vol_ratio_15m > 150:
        ...
        if swept_ssl: score_buy += 1
        if swept_bsl: score_sell += 1
    else: notes.append(f"[{vol_status_15m}]")
    if mfi_15m < 20: score_buy += 2; notes.append(f"[BUY] MFI 15m OS {mfi_15m:.1f} <20")
    elif mfi_15m < 40: score_buy += 1; notes.append(f"[BUY] MFI 15m Murah {mfi_15m:.1f}")
    elif mfi_15m > 80: score_sell += 2; notes.append(f"[SELL] MFI 15m OB {mfi_15m:.1f} >80")
    elif mfi_15m > 60: score_sell += 1; notes.append(f"[SELL] MFI 15m Mahal {mfi_15m:.1f}")
    else: notes.append(f"[NEUTRAL] MFI 15m {mfi_15m:.1f}")
    if mfi_daily < 30: notes.append(f"[HTF BUY] MFI Daily OS {mfi_daily:.1f}")
    elif mfi_daily > 70: notes.append(f"[HTF SELL] MFI Daily OB {mfi_daily:.1f}")
    if is_london or is_ny: notes.append(f"[OK] {kz}")
    else: notes.append(f"[HATI-HATI] {kz}")
    if "Bull" in ema_bias: score_buy += 1
    elif "Bear" in ema_bias: score_sell += 1
    ...
    else: bias = "WAIT"; strength = "C - No Trade"
    s_buy = 0; s_sell = 0; s_notes = []
    if mfi_5m < 20: s_buy += 3; s_notes.append(f"[SCALP BUY] MFI 5m OS {mfi_5m:.1f}")
    elif mfi_5m < 35: s_buy += 1
    elif mfi_5m > 80: s_sell += 3; s_notes.append(f"[SCALP SELL] MFI 5m OB {mfi_5m:.1f}")
    elif mfi_5m > 65: s_sell += 1
    if vol_ratio_5m > 180:
        if swept_5m_bear: s_buy += 2
        if swept_5m_bull: s_sell += 2
        s_notes.append(f"[SCALP BANDAR] {vol_status_5m}")
    if swept_5m_bear: s_buy += 2; s_notes.append("[SCALP BUY] Sweep Low 5m")
    if swept_5m_bull: s_sell += 2; s_notes.append("[SCALP SELL] Sweep High 5m")
    if fvg_bull_5: s_buy += 1
    if fvg_bear_5: s_sell += 1
    if is_discount: s_buy += 1
    else: s_sell += 1
    if is_london or is_ny: s_notes.append(f"[SCALP OK] {kz}")
    else: s_buy -= 1; s_sell -= 1; s_notes.append(f"[SCALP WAIT] {kz}")
    if s_buy >= s_sell + 3: s_bias = "BUY SCALP 5-10M"; s_strength = "A+"
    elif s_sell >= s_buy + 3: s_bias = "SELL SCALP 5-10M"; s_strength = "A+"
    elif s_buy > s_sell: s_bias = "BUY SCALP 5-10M"; s_strength = "B"
    elif s_sell > s_buy: s_bias = "SELL SCALP 5-10M"; s_strength = "B"
    else: s_bias = "WAIT SCALP"; s_strength = "C"
    if bias == "BUY":
    ...
    data = {
        ...
        "y_high": y_high, "y_low": y_low, "weekly_high": weekly_high, "weekly_low": weekly_low, "weekly_50": weekly_50,
        "dxy": dxy, "us10y": us10y, "real_yield": real_yield,
        "bsl": bsl, "ssl": ssl, "swept_bsl": swept_bsl, "swept_ssl": swept_ssl,
        "swept_5m_bull": swept_5m_bull, "swept_5m_bear": swept_5m_bear,
        "q1": q1, "q2": q2, "q3": q3, "time": now_ksa_str, "time_full": now_ksa_full, "kz": kz, "ema_bias": ema_bias,
        "mfi_15m": mfi_15m, "mfi_5m": mfi_5m, "mfi_daily": mfi_daily,
        "vol_ratio_15m": vol_ratio_15m, "vol_ratio_5m": vol_ratio_5m,
        "vol_status_15m": vol_status_15m, "vol_status_5m": vol_status_5m,
        "curr_vol_15": curr_vol_15, "avg_vol_15": avg_vol_15,
    }
    analisa = {
        "bias": bias, "strength": strength, "score_buy": score_buy, "score_sell": score_sell,
        "entry": entry, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3, "rr1": rr1, "rr2": rr2,
        "notes": notes, "premium_status": premium_status, "is_discount": is_discount,
    }
    scalp = {
        "bias": s_bias, "strength": s_strength, "score_buy": s_buy, "score_sell": s_sell,
        "entry": s_entry, "sl": s_sl, "tp1": s_tp1, "tp2": s_tp2,
        "rr": abs(s_tp1-s_entry)/abs(s_entry-s_sl) if s_entry != s_sl else 0,
        "notes": s_notes, "hold": "5-10 menit", "mfi_5m": mfi_5m, "mfi_15m": mfi_15m,
    }
    CACHE["data"] = data; CACHE["analisa"] = analisa; CACHE["scalp"] = scalp; CACHE["time"] = now_ts
    return data, analisa, scalp

def run_backtest_swing_text():
    """Backtest swing 150 hari - hemat credit, keluar text buat Telegram"""
    try:
        xau = td_get("XAU/USD", "time_series", "1day", 150)
        if "error" in xau or "values" not in xau:
            return f"\nBACKTEST GAGAL\nError: {xau.get('error', xau)}\nCek TD_API_KEY & credit TwelveData\n"
        values = list(reversed(xau["values"]))
        trades = []; balance = 10000; risk = 0.01
        for i in range(20, len(values)-1):
            today = values[i]; yest = values[i-1]
            ...
            if is_discount: score_buy += 2
            else: score_sell += 2
            if live < float(yest["low"]): score_buy += 2
            if live > float(yest["high"]): score_sell += 2
            if mfi < 20: score_buy += 2
            elif mfi > 80: score_sell += 2
            if live > sum(float(v["close"]) for v in values[max(0,i-20):i])/20: score_buy += 1
            else: score_sell += 1
            if score_buy >= score_sell+3: bias = "BUY"
            elif score_sell >= score_buy+3: bias = "SELL"
            else: continue
            ...
            next_day = values[i+1]
            nh = float(next_day["high"]); nl = float(next_day["low"])
            result = "FLOAT"
            if bias=="BUY":
                if nl <= sl: result = "SL"
                elif nh >= tp: result = "TP"
            else:
                if nh >= sl: result = "SL"
                elif nl <= tp: result = "TP"
            if result != "FLOAT":
                rr = abs(tp-entry)/abs(entry-sl) if entry!=sl else 0
                profit = balance*risk*rr if result=="TP" else -balance*risk
                balance += profit
                trades.append({"date": today["datetime"], "bias": bias, "score": f"{score_buy}:{score_sell}", "mfi": round(mfi,1), "entry": entry, "sl": sl, "tp": tp, "result": result, "rr": round(rr,2), "balance": round(balance,2)})
        ...
        last5 = "\n".join([f"{t['date']} {t['bias']} {t['result']} RR{t['rr']} Bal${t['balance']}" for t in trades[-5:]])
        return f"""BACKTEST SWING REAL - 13 BAHAN KSA
Periode: 150 hari 1day REAL (5 bulan)
Total Trade: {len(trades)} | Win: {win} | Loss: {loss} | Winrate: {winrate}%
Balance: $10000 -> ${round(balance,2)}

5 Trade Terakhir:
{last5}

Sumber: TwelveData REAL fetch, bukan dummy
Waktu: {datetime.now(KSA_TZ).strftime('%Y-%m-%d %H:%M KSA')}"""
    except Exception as e:
        return f"\nBACKTEST ERROR: {e}\n"

def run_backtest_scalp_text():
    """Backtest scalping 200 candle 5m - hemat credit"""
    try:
        xau_5m = td_get("XAU/USD", "time_series", "5min", 200)
        if "error" in xau_5m or "values" not in xau_5m:
            return f"\nBACKTEST SCALP GAGAL\nError: {xau_5m.get('error', xau_5m)}\nCek credit\n"
        values = list(reversed(xau_5m["values"]))
        trades = []
        for i in range(20, len(values)-1):
            today = values[i]; live = float(today["close"])
            mfi, vol_ratio, _, _, _ = calc_mfi_volume(values[max(0,i-15):i+1], 14)
            score_buy = 0; score_sell = 0
            if mfi < 20: score_buy += 3
            elif mfi > 80: score_sell += 3
            last5 = values[max(0,i-5):i]
            if last5:
                last5_high = max(float(v["high"]) for v in last5)
                last5_low = min(float(v["low"]) for v in last5)
                if live < last5_low: score_buy += 2
                if live > last5_high: score_sell += 2
            if vol_ratio > 180:
                if score_buy>0: score_buy += 1
                if score_sell>0: score_sell += 1
            if score_buy >= score_sell+3: bias = "BUY SCALP"
            elif score_sell >= score_buy+3: bias = "SELL SCALP"
            else: continue
            entry = live; sl = live-5 if bias.startswith("BUY") else live+5; tp = live+8 if bias.startswith("BUY") else live-8
            nxt = values[i+1]; nh = float(nxt["high"]); nl = float(nxt["low"])
            result = "FLOAT"
            ...
            if result != "FLOAT":
                trades.append({"datetime": today["datetime"], "bias": bias, "score": f"{score_buy}:{score_sell}", "mfi_5m": round(mfi,1), "vol_ratio": round(vol_ratio,0), "entry": entry, "sl": sl, "tp": tp, "result": result})
        win = len([t for t in trades if t["result"]=="TP"])
        winrate = round(win/len(trades)*100,1) if trades else 0
        last5 = "\n".join([f"{t['datetime']} {t['bias']} {t['result']} MFI{t['mfi_5m']} Vol{t['vol_ratio']}%" for t in trades[-5:]])
        return f"""BACKTEST SCALP 5-10M REAL - 13 BAHAN KSA
Periode: 200 candle 5m REAL (~1 hari)
Total: {len(trades)} | Win: {win} | Loss: {len(trades)-win} | Winrate: {winrate}%
...
Hold: {s['hold']}"""
    return f"""XAUUSD FINAL 13 BAHAN KSA | {d['time']} | {d['kz']}
------------------------------
...
def keyboard():
    ...
    mk.add(
        ...
        telebot.types.KeyboardButton("Backtest SCALP 5M"),
...
@bot.message_handler(commands=['start','xau','myid','backtest','backtest_scalping'])
def start_cmd(m):
    cmd = m.text.lower()
    if "backtest_scalping" in cmd or "backtest_scalp" in cmd:
        bot.send_message(m.chat.id, "⏳ Running backtest scalping 5M REAL (200 candle)... tunggu 10 detik")
        txt = run_backtest_scalp_text()
        bot.send_message(m.chat.id, txt, parse_mode=None)
        return
    if "backtest" in cmd:
        bot.send_message(m.chat.id, "⏳ Running backtest swing REAL (150 hari)... tunggu 10 detik")
        txt = run_backtest_swing_text()
        bot.send_message(m.chat.id, txt, parse_mode=None)
        return
    bot.send_message(m.chat.id, "Bot XAU FINAL TERLENGKAP KSA Madinah Ready - 13 Bahan + Vol + MFI + Scalp 5-10M + Backtest REAL (FIX):", reply_markup=keyboard())
    d,a,s = get_all_data()
    if d:
        bot.send_message(m.chat.id, build_message(d,a,s), parse_mode=None, reply_markup=inline_buttons())
...
def handle_all(m):
    ...
    if text in ["XAU 13 BAHAN KSA","VOLUME & MFI REAL"]:
        ...
        txt = f"""SCALPING 5-10 MENIT KSA | {d['time']} | {d['kz']}
...
Hold: {s['hold']}"""
        bot.send_message(m.chat.id, txt, parse_mode=None, reply_markup=keyboard())
    elif text in ["BUY","SELL"]:
        a["bias"] = text
        bot.send_message(m.chat.id, build_message(d,a,s), parse_mode=None, reply_markup=inline_buttons())
    elif text == "Killzone KSA":
        now = datetime.now(KSA_TZ)
        txt = f"""Killzone Madinah KSA GMT+3:
...
Waktu: {now.strftime('%Y-%m-%d %H:%M:%S KSA (Madinah)')}"""
        ...
        bot.send_message(m.chat.id, "⏳ Running backtest swing REAL 150 hari... tunggu 10 detik, jangan spam")
        txt = run_backtest_swing_text()
        bot.send_message(m.chat.id, txt, parse_mode=None, reply_markup=keyboard())
    elif text == "Backtest SCALP 5M":
        bot.send_message(m.chat.id, "⏳ Running backtest scalp 5M REAL 200 candle... tunggu 10 detik")
        txt = run_backtest_scalp_text()
        bot.send_message(m.chat.id, txt, parse_mode=None, reply_markup=keyboard())
    elif text == "ID Saya":
        bot.send_message(m.chat.id, f"ID lu: {m.chat.id}\nWaktu lu: {d['time_full']}\nCopy ID ini ke Render ENV TARGET_CHAT_ID", parse_mode=None, reply_markup=keyboard())
...
def callback(call):
    ...
    if call.data == "BUY":
        a["bias"] = "BUY"
        bot.send_message(call.message.chat.id, build_message(d,a,s), parse_mode=None, reply_markup=inline_buttons())
    elif call.data == "SELL":
        a["bias"] = "SELL"
        bot.send_message(call.message.chat.id, build_message(d,a,s), parse_mode=None, reply_markup=inline_buttons())
...
@app.route('/')
def home():
    return "Bot XAU FINAL TERLENGKAP KSA FIX BACKTEST - Ready - /check /backtest /backtest_scalping /getid /getupdates"
...
def backtest_real():
    txt = run_backtest_swing_text()
    # return both text and json for browser
...
if __name__ == "__main__":
    ...
    print("Bot FINAL TERLENGKAP KSA FIX BACKTEST jalan")
    while True:
        try:
